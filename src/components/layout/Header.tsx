      ...
      <div className="container">
        <div className="flex items-center justify-between h-16 md:h-24">
          ...
          <nav className="hidden md:flex items-center gap-9" role="navigation" aria-label="Main navigation">
                  ...
                  >
                    {services.map(service => (
                      <Link
                        ...
                        role="menuitem"
                      >
                        <span className="font-mono text-xs text-ash group-hover:text-lime transition-colors">{service.number}</span>
            ...
            <NavLink to="/how-we-work" className={navLinkClass}>How We Work</NavLink>
          ...
          <div className="hidden md:flex items-center gap-3">
            <Button to="/contact" size="sm">Book a 20-minute call</Button>
          </div>
          ...
          >
            <span className={`block w-6 h-px bg-bone transition-all duration-200 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`block w-6 h-px bg-bone transition-all duration-200 ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`block w-6 h-px bg-bone transition-all duration-200 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>
      </div>
        ...
        {menuOpen && (
          ...
          >
            <div className="container py-8 flex flex-col gap-1">
              ...
              <div className="mt-6">
                <Button to="/contact" size="md" className="w-full">Book a 20-minute call</Button>
              </div>
            </div>
          </motion.div>
        )}
...
function MobileNavLink({ to, label, sub }: { to: string; label: string; sub?: string }) {
  return (
    ...
    >
      {sub && <span className="font-mono text-xs text-ash w-6 flex-shrink-0">{sub}</span>}
